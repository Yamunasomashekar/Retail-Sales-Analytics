...
----explanation-----
----verify column_names data_types-----
...
--check how many rows have whitespace issues
SELECT COUNT(*) AS RowsWithWhitespace
FROM SuperstoreSales
WHERE Product_Name != TRIM(Product_Name);

-- See exactly which Product_IDs have more than one Product_Name, and what those names are--
SELECT Product_ID, Product_Name, COUNT(*) AS TimesUsed
FROM SuperstoreSales
WHERE Product_ID IN (
    SELECT Product_ID
...
/*conclusion
we are grouping based on the combination of product_id and product_name,same product_id
but different product_names*/
